#
# Heuristic search techniques: Puzzle
#
from ai_ch07.simpleai.search import astar, SearchProblem
...
for i, (action, state) in enumerate(result.path()):
    ...
    if action == None:
        ...
        ai07_6_url1.insert(ctr, 'Initial configuration')
        ctr = ctr + 1
    elif i == len(result.path()) - 1:
        print('After moving', action, 'into the empty space. Goal achieved!')
        ai07_6_url1_str = 'After moving', action, 'into the empty space. Goal achieved!'
        ai07_6_url1.insert(ctr, ai07_6_url1_str)
        ctr = ctr + 1
    else:
        print('After moving', action, 'into the empty space')
        ai07_6_url1_str = 'After moving', action, 'into the empty space'
        ai07_6_url1.insert(ctr, ai07_6_url1_str)
        ctr = ctr + 1

    print(state)
    ai07_6_url1.insert(ctr, state)
    ctr = ctr + 1
